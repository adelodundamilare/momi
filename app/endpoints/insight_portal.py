...
async def get_insight_portal(
    ...
    if conversation_id:
        try:
            chat_context = insight_portal_service.create_chat_context_for_insights(
                db, conversation_id, current_user.id
            )
            logger.info(f"Using chat context from conversation {conversation_id} for insights")
        except ValueError as e:
            logger.warning(f"Failed to extract chat context: {str(e)}")
            chat_context = None
    ...
    if ai_generated_data.error == "insufficient_context":
        raise HTTPException(
            status_code=400,
            detail=ai_generated_data.error_message or "Unable to determine specific ingredient from conversation. Please provide more specific details about which ingredient you want market insights for."
        )
